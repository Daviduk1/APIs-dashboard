...
allLinks.forEach(link => {
    link.addEventListener('click', () => {
        ...
        if (linkText === 'posts') {
            selectedItemArr = postsArr
            selectedItemFunc = showPosts
            showPosts(postsArr)
        } else if (linkText === 'comments') {
            selectedItemArr = commentsArr
            selectedItemFunc = showComments
            showComments(commentsArr)
        } else if (linkText === 'albums') {
            selectedItemArr = albumsArr
            selectedItemFunc = showAlbums
            showAlbums(albumsArr)
        } else if (linkText === 'photos') {
            selectedItemArr = photosArr
            selectedItemFunc = showPhotos
            showPhotos(photosArr)
        } else if (linkText === 'todos') {
            selectedItemArr = todosArr
            selectedItemFunc = showTodos
            showTodos(todosArr)
        } else {
            selectedItemArr = usersArr
            selectedItemFunc = showUsers
            showUsers(usersArr)
        }
    })
})
...
searchBar.oninput = (evt) => {
    let value = evt.target.value
        let filtered = selectedItemArr.filter(item => {
            for (let key in item) {
        ...
        selectedItemFunc(filtered)
}

let sidebar = document.querySelector('#app aside')
